...
function HistoryPanel({
  ...
  const modeColors: Record<string, string> = {
    compare: "bg-indigo-500/15 text-indigo-300",
    synthesize: "bg-amber-500/15 text-amber-300",
    pipeline: "bg-teal-500/15 text-teal-300",
  };
  return (
    <section className="mt-12 rounded-2xl border border-slate-800 bg-slate-900/40 p-5 shadow-xl shadow-black/30">
      <h2 className="mb-4 text-base font-semibold text-slate-100">Run history</h2>
      {loading ? (
        <p className="text-sm text-slate-500">Loading…</p>
      ) : runs.length === 0 ? (
        <p className="text-sm text-slate-500">No saved runs yet. Runs are saved after each completion.</p>
      ) : (
          ...
          {runs.map((run) => (
            <li key={run.id}>
              <button
                ...
                className="w-full rounded-xl border border-slate-800 bg-slate-950/50 px-4 py-3 text-left transition hover:border-slate-700 hover:bg-slate-900/60"
              >
                <div className="mb-1 flex flex-wrap items-center gap-2">
                  <span
                    className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${modeColors[run.mode] ?? "bg-slate-700 text-slate-400"}`}
                  >
                    {run.mode}
                  </span>
                  <span className="text-xs text-slate-500">{formatTs(run.created_at)}</span>
                </div>
                <p className="truncate text-sm font-medium text-slate-200">{run.prompt}</p>
                <p className="mt-0.5 truncate text-xs text-slate-500">{run.summary}</p>
              </button>
            </li>
          ))}
        </ol>
...
export default function App() {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-6xl px-4 pb-16 pt-10 sm:px-6 lg:px-8">
      {/* Header */}
      <header className="mb-10">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm leading-relaxed text-slate-400">
              Compare models side by side, synthesize the strongest successful answers, or run
              a sequential draft → critique → improve → verify → final workflow.
            </p>
          </div>
          ...
          <div className="shrink-0 pt-1">
            {authLoading ? null : user ? (
              ...
              <button
                ...
                disabled={signingIn}
                className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-sm font-medium text-slate-200 transition hover:border-slate-600 hover:bg-slate-800 disabled:opacity-50"
              >
        ...
        {user ? null : (
          <p className="rounded-lg border border-slate-800 bg-slate-900/40 px-3 py-2 text-xs text-slate-500">
            Sign in to save runs to your history across sessions. Google sign-in uses a full-page
            redirect so it works when strict security headers block popups.
          </p>
        )}
        {authError ? (
      ...
      <section className="mb-8 rounded-2xl border border-slate-800 bg-slate-900/40 p-5 shadow-xl shadow-black/30">
        ...
        <textarea
          ...
          className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950/80 px-3 py-2.5 font-sans text-sm text-slate-100 placeholder:text-slate-600 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
        />

        {/* Mode selector */}
        <div className="mt-5">
          <p className="mb-2 text-sm font-medium text-slate-300">Mode</p>
          <div className="grid gap-3 md:grid-cols-3">
            {(
              [
                ["compare", "Compare only", "Run selected models independently."],
                [
                  "synthesize",
                  "Compare + synthesize",
                  "Score, judge, and merge successful answers.",
                ],
                [
                  "pipeline",
                  "Joint pipeline",
                  "Draft → critique → improve → verify → final.",
                ],
        ...
        {mode === "pipeline" ? (
          <div className="mt-5 rounded-xl border border-slate-800 bg-slate-950/40 p-4">
            <p className="mb-3 text-sm font-medium text-slate-300">Pipeline models</p>
            {modelsError ? (
              <p className="text-sm text-red-400">{modelsError}</p>
            ) : availableModels.length === 0 ? (
              <p className="text-sm text-slate-500">No available models yet.</p>
            ) : (
          ...
          <div className="mt-5">
            <p className="mb-2 text-sm font-medium text-slate-300">Models</p>
            {modelsError ? (
              <p className="text-sm text-red-400">{modelsError}</p>
            ) : models.length === 0 ? (
                ...
                {models.map((m) => (
                  <li key={m.model_id}>
                    ...
                    >
                      ...
                      <span className="font-mono text-xs opacity-70">{m.provider}</span>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          <button
            ...
            className="rounded-lg bg-indigo-500 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-900/40 transition hover:bg-indigo-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400 disabled:shadow-none"
          >
            {loading
              ? mode === "pipeline"
                ? "Running pipeline…"
          ...
          {!anyConfigured && models.length > 0 ? (
            <span className="text-sm text-amber-300/90">
              Add API keys in{" "}
              <code className="font-mono text-xs">backend/.env</code> to enable models.
            </span>
          ) : null}
          {user ? (
            <span className="text-xs text-slate-500">Runs saved to your account.</span>
          ) : (
